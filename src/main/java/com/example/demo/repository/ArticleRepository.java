...
public interface ArticleRepository extends JpaRepository<Article,Long> {
    ...
    List<Article> findByStatusIn(List<String> status);
    ...
    List<Article> findByUserAndStatusIn(List<String> status, User user);
     ...
     * @return
     */
    @Query(value = "SELECT user_id FROM article WHERE status = 'publish' AND (DATE_SUB(CURDATE()" +
            ",INTERVAL 30 DAY) <= date(create_time))  GROUP BY user_id ORDER BY count(id) DESC LIMIT 10", nativeQuery = true)
     ...
     * @return
     */
    @Query(value = "SELECT count(id) as count FROM article  WHERE status = 'publish' AND (DATE_SUB(CURDATE()" +
            ",INTERVAL 30 DAY) <= date(article.create_time))  GROUP BY user_id ORDER BY count DESC LIMIT 10", nativeQuery = true)
