$(function() {
  $("#choseimg").on("change", function() {
    ...
    if (!fileFormat.match(/.png|.jpg|.jpeg/)) {
      ...
      $("#jiahao").html("");
      $("#jiahao").append('<img id="imgs" src="' + src + '">');
  ...
  $$(".btn-up").on("click", function() {
    ...
    $.ajax({
        url:"/article/add",
        type:"get",
        data:{"type":type,"city":city,"tarea":tarea,"title":title},
  ...
  dropContainer.on("dragover", e => {
    ...
    let bool = $(e.target).is(circle) || $.contains(circle[0], e.target),
      distance =
        calculateDistance(circle, dropContainer, e.pageX, e.pageY) > 1
          ? 1
          : calculateDistance(circle, dropContainer, e.pageX, e.pageY);
    mouse = {
      x: e.pageX,
      y: e.pageY
    };
  ...
  function shootDot(emitter, move) {
    let angle =
        Math.atan2(move[1].y - move[0].y, move[1].x - move[0].x) *
  ...
  function dropDot(emitter, bounding, dotSize) {
    let size = dotSize.min + Math.random() * (dotSize.max - dotSize.min),
      ...
      dot =
        dotPoolDrop[
          dotIndexDrop === dotQuantityDrop ? (dotIndexDrop = 0) : dotIndexDrop++
        ],
      offset = emitter.offset(),
